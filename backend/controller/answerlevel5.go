...
type AnswerRequest5 struct {
	Answer string `json:"answer" binding:"required"`
}

// AnswerResponse ใช้ส่งผลลัพธ์กลับไปยังผู้ใช้
type AnswerResponse5 struct {
	Correct bool   `json:"correct"`
	Message string `json:"message"`
}

// ValidateAnswerLevel5 ฟังก์ชันสำหรับตรวจสอบคำตอบ
func ValidateAnswerLevel5(c *gin.Context) {
	var request AnswerRequest5

	// ตรวจสอบว่า JSON ที่ส่งมานั้นถูกต้องหรือไม่
	if err := c.ShouldBindJSON(&request); err != nil {
		c.JSON(http.StatusBadRequest, AnswerResponse5{
			Correct: false,
	...
	// คำตอบที่ถูกต้อง
	correctAnswer := "Computer Engineering"
	...
	if strings.TrimSpace(strings.ToLower(request.Answer)) == strings.TrimSpace(strings.ToLower(correctAnswer)) {
		c.JSON(http.StatusOK, AnswerResponse5{
			Correct: true,
