...
type AnswerRequest6 struct {
	Answer string `json:"answer" binding:"required"`
}

// AnswerResponse ใช้ส่งผลลัพธ์กลับไปยังผู้ใช้
type AnswerResponse6 struct {
	Correct bool   `json:"correct"`
	Message string `json:"message"`
}

// ValidateAnswerLevel3 ฟังก์ชันสำหรับตรวจสอบคำตอบ
func ValidateAnswerLevel6(c *gin.Context) {
	var request AnswerRequest6

	// ตรวจสอบว่า JSON ที่ส่งมานั้นถูกต้องหรือไม่
	if err := c.ShouldBindJSON(&request); err != nil {
		c.JSON(http.StatusBadRequest, AnswerResponse6{
			Correct: false,
